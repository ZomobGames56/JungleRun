...
public class PlayerController_Jumping : MonoBehaviour
{
    ...
    void HandleFallCheck()
    {
        ...
        else if (transform.position.y < -1.2f)
        {
            ...
            if (!onceCalled)
            {
                if (splashObj != null)
                {
                    Instantiate(splashObj, transform).transform.SetParent(null);
                    // Instantiate(splashObj, new Vector3(transform.position.x, transform.position.y + 4f, transform.position.z), Quaternion.identity);
                }
                SoundManager_Jumping.Instance.PlayLoseSound("drown");
                onceCalled = true;
            }
        }
    }
