...
public class GameManager_BridgeLevel : MonoBehaviour
{
    public static GameManager_BridgeLevel Instance { get; set; }
    ...
    public bool powerUpAllowed = false;
    public bool gliderCollected = false;
    public bool startGlide = false;
    ...
    public static event Action<bool, string> playerMovementFloatingBridge;
    public static event Action<GameObject, Animator> updatePlayerMoveFloatingBridge;
    public static event Action<bool, string> playerMovementIce;
    public static event Action<GameObject, Animator> updatePlayerMoveIce;
    public static event Action bridgeSpawnerFloatingBridge;
    public static event Action bridgeSpawnerIce;
    public static event Action<bool> coinSpawnFloatingBridge;
    public static event Action<bool> coinSpawnIce;
    ...
    void Start()
    {
        ...
        else
        {
            ...
            if (levelType == "FloatingBridge" || levelType == "Horror")
            {
                ...
                coinSpawnFloatingBridge?.Invoke(true);
                bridgeSpawnerFloatingBridge?.Invoke();
            }
            else if (levelType == "Ice")
            {
    ...
    public void UpdateChild(GameObject a, GameObject b)
    {
        ...
        if (levelType == "Ice")
        {
            updatePlayerMoveIce?.Invoke(playerBody, animator);
        }
        else
        {
            updatePlayerMoveFloatingBridge?.Invoke(playerBody, animator);
        }
    }
    ...
    {
        // coinsCurr += amount;
        // if (coinsCurr >= coinsBtwPowerups)
        // {
        //     coinsBtwPowerups += coinsCurr;
        //     powerUpAllowed = true;
        // }
    ...
    {
        speedModifier += 0.1f * speedModifier;
        Debug.Log(speedModifier);
        if (speedModifier > 1.75f) speedModifier = 1.75f;
    }

    public void EnableSnowScreen(GameObject obj)
    {
        StartCoroutine(SlowDisappear(obj));
    }

    IEnumerator SlowDisappear(GameObject obj)
    {
        obj.SetActive(true);
    ...
    //     startScreen.SetActive(false);
    //     inGameScreen.SetActive(true);
    //     player.transform.GetChild(0).GetComponent<Animator>().SetTrigger("Run");
    //     startTutorial = true;
    //     PlayerPrefs.SetInt("Tutorial - " + levelType, startTutorial ? 1 : 0);
    //     PlayerPrefs.Save();
    ...
    public void BackButton(string screen)
    {
        ...
        settingschild.DOScale(0f, 0.3f).SetUpdate(true).OnComplete(() => settingsScreen.SetActive(false));
    ...
    public void MeButton()
    {
        ...
        DOTween.Kill(Camera.main.transform);
        Camera.main.transform.DOMove(charScreenCamPos.position, 1f).SetEase(Ease.InOutSine);
        Camera.main.transform.DORotate(charScreenCamPos.eulerAngles, 1f).SetEase(Ease.InOutSine);
        ...
        Sequence seq = DOTween.Sequence();
        seq.Append(rectTransform1.DOAnchorPos(new Vector2(0, 0), 1f).SetEase(Ease.OutExpo));
        seq.Join(rectTransform2.DOAnchorPos(new Vector2(0, 50), 1f).SetEase(Ease.OutExpo));
        seq.OnComplete(() => isTransitioning = false);
    }
    ...
    public void ExploreButton()
    {
        ...
        rectTransform1
            .DOAnchorPos(new Vector2(0, -136.0349f), 0.5f)
    ...
    public void ConfirmNoButton()
    {
        ...
        rect.DOScale(0f, 0.3f).SetUpdate(true).OnComplete(() =>
        {
            purchaseConfirmScreen.SetActive(false);
            isTransitioning = false;
        });
